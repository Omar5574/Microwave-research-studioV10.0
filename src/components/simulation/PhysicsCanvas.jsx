  ...
  useEffect(() => {
    ...
    // === TWO-CAVITY KLYSTRON (FINAL: Smooth Motion + Dynamic Current) ===
      if (deviceId === 'klystron2') {
          ...
          const drift_angle_rad = (omega_real * L_meters) / v0_real; 
          ...
          const output_glow_intensity = Math.min(1, Math.abs(J1_X) * 1.7); 
          ...
          if (running) {
            ...
            particlesRef.current.forEach((p, i) => {
                // A. Buncher Interaction (SMOOTHER VELOCITY)
          ...
          ctx.fillText(`Bunching Param (X): ${X_real.toFixed(3)}`, 120, 30);
          ...
          ctx.fillStyle = statusColor; ctx.fillText(statusText, 120, 50);
      ...
      // === MULTI-CAVITY KLYSTRON (UPDATED VISUALS) ===
         ...
         // --- 2. Particle Logic (Dynamic Density) ---
          ...
          ctx.fillText(`Gain: ${total_gain_db} dB`, 120, 30);
          ctx.font = '12px monospace';
          ctx.fillStyle = '#94a3b8';
          ctx.fillText(`(Visuals Enhanced for Visibility)`, 120, 45);
